...
*/
        if(!empty($_POST['subdir'])) {
            $this->updir=$this->updir.DS.$_POST['subdir'];
...
*/
     ...
     function post(){
         ...
         if(isset($_POST['image'])){
             ...
             $data=base64_decode($img);
             $im=$this->dir.DS.$this->filename.'.'.$ext;
             $fp=file_put_contents($this->dir.DS.$this->filename.'.'.$ext,$data);
             if($fp!==false) { 
                 $this->data( array(
                    'success'=>true,
            ...
            $this->data(array(
                'success'=>false,
                'error'=>'Periksa permission folder',
            ));
            return;
         }
         $this->data(array(
